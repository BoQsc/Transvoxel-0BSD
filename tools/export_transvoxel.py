...
For now, each case is its own class. That is less compressed than the standard
73 transition equivalence classes, but it preserves behavior and keeps the
exporter mechanically generated from the canonical JSON.
"""
...
def main() -> int:
    ...
    transition = load_json(GENERATED / "transition_tables.json")
    transvoxel = {
        "schema": "boqsc.transvoxel_tables.v1",
        "license": "0BSD",
        "status": "transvoxel-table-export; not official Transvoxel.cpp byte/table clone",
        "table_contract": "case index -> class index -> class data -> vertex refs + triangles",
        "compression": "direct one-class-per-case; no 73-class compression yet",
        "regular": build_table(regular, "regular"),
        "transition": build_table(transition, "transition"),
        "source_tables": {

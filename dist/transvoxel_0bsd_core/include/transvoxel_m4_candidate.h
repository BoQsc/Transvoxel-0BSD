 ...
 * Explicit M4 runtime API.
 *
 * This path consumes generated/official_topology_candidate_tables.h. The
 * normal tv_build_transition_cell() default backend is exported from the same
 * clean-room M4 topology source, while this API remains available for explicit
 * M4 sample-count, face-frame, mapped, and oriented calls. It does not claim
 * official Transvoxel.cpp table identity or byte compatibility.
 */
...
/* M4 uses public transition samples 0..12 only. The normal transvoxel.h ABI
 * still exposes synthetic sample 13 for older 14-sample callers; this M4 path
 * intentionally does not use that synthetic center sample.
 */

...
int main(void) {
    ...
    if (default_info.vertex_count != DEMO_M4_EXPECTED_VERTICES
        || default_info.triangle_count != DEMO_M4_EXPECTED_TRIANGLES) {
        printf(
            "default_count_mismatch vertices=%d triangles=%d\n",
    ...
    if (m4_info.vertex_count != default_info.vertex_count
        || m4_info.triangle_count != default_info.triangle_count) {
        printf("m4_no_longer_matches_default\n");
        return 1;
    }
    ...
    printf(
        "m4 package backend case=%d default_vertices=%d default_triangles=%d m4_vertices=%d m4_triangles=%d same_as_default=1 restored_default=1 custom_after=0\n",
        DEMO_CASE_INDEX,
        default_info.vertex_count,
        default_info.triangle_count,
        m4_info.vertex_count,
